...
class SensorReadingController
{
    ...
    public function store(Request $request)
    {
        ...
        // 3. Simpan ke database PostgreSQL/SQLite
        $id = DB::table('sensor_readings')->insertGetId([
            'pond_id' => $validated['pond_id'] ?? 1, // Default pond 1
    ...
    // Fungsi untuk memperbarui data sensor (Update)
    public function update(Request $request, $id)
    {
        ...
        $existing = DB::table('sensor_readings')->where('id', $id)->first();
        if (!$existing) {
            return response()->json(['status' => 'error', 'message' => 'Data tidak ditemukan'], 404);
        }

        // Prediksi ulang berdasarkan data baru
        $waterCondition = $existing->water_condition;
        ...
        try {
            ...
            // Abaikan error ML service
        }

        DB::table('sensor_readings')->where('id', $id)->update([
            'temperature' => $validated['temperature'],
            'ph' => $validated['ph'],
            'do' => $validated['do'],
            'nh3' => $validated['nh3'],
    ...
    // Fungsi untuk menghapus data sensor (Delete)
    public function destroy($id)
    {
        $deleted = DB::table('sensor_readings')->where('id', $id)->delete();
        if (!$deleted) {
            return response()->json(['status' => 'error', 'message' => 'Data tidak ditemukan'], 404);
        }
