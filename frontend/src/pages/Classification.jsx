...
const STATUS_THEME = {
  ...
  'Tidak Diketahui': {
    ...
    paramBadgeBg: 'bg-slate-100',
    paramBadgeText: 'text-slate-800',
    rekoBg: 'bg-slate-50',
    rekoBorder: 'border-slate-300',
    rekoText: 'text-slate-900',
    rekoAccent: 'text-slate-700',
  },
};

/* ──────────────────────────────────────────────
   Helper: Normalize water condition text to theme keys
   ────────────────────────────────────────────── */
function getNormalizedCondition(cond) {
  if (!cond) return 'Tidak Diketahui';
  const name = cond.toLowerCase().trim();
  if (name.includes('optimal') || name.includes('aman') || name.includes('baik') || name.includes('sehat')) {
    return 'Optimal';
  }
  if (name.includes('siaga') || name.includes('waspada') || name.includes('peringatan') || name.includes('hati-hati')) {
    return 'Siaga';
  }
...
export default function Classification() {
  ...
  const normalizedKey = getNormalizedCondition(kondisi);
  const theme = STATUS_THEME[normalizedKey] || STATUS_THEME['Tidak Diketahui'];
  const ShieldIcon = theme.icon;
  const StatusIcon = theme.statusIcon;

  // Dummy feeding recommendation based on active water condition
  let recommendation = latest?.recommendation || 'Belum ada rekomendasi. Kirim data sensor untuk mendapatkan analisis.';
  if (latest) {
    if (normalizedKey === 'Optimal') {
      recommendation = 'Kondisi air terpantau aman dan optimal. Direkomendasikan untuk memberikan pakan penuh (100%) sesuai dengan takaran harian normal.';
    } else if (normalizedKey === 'Siaga') {
      recommendation = 'Kondisi air terpantau waspada/siaga. Direkomendasikan untuk membatasi pemberian pakan hingga 50% dari takaran normal guna menghindari penumpukan sisa pakan.';
    } else if (normalizedKey === 'Kritis') {
      recommendation = 'Kondisi air dalam keadaan bahaya atau mematikan! Direkomendasikan untuk menghentikan pemberian pakan secara total (0%) sementara waktu sampai parameter air kembali stabil.';
    }
  }
